...
gauss_reduce(Matrix aug, unsigned int precision, bool partial_pivot) {
  ...
  for (size_t pivot_col = 1; pivot_col <= n_cols_a; pivot_col++) {
    ...
    for (size_t i = pivot_row + 1; i <= n_rows; i++) {
      ...
      if (matrix_row_add_row(aug, i, pivot_row, multiplier, precision) < 0)
        return -1;
    }
  ...
  for (size_t b_col = 1; b_col <= n_cols_b; b_col++) {
    for (size_t i = n_rows; i > 0; i--) {
      ...
      for (size_t j = i + 1; j <= n_cols_a; j++) {
        ...
        x_sum -= fl(fl(t_value, precision) * fl(x_val, precision), precision);
      }

      if ((matrix_get(g_reduced, i, i, &t_value)) < 0)
        goto fail;

      if (matrix_set(x, i, b_col, fl(x_sum / t_value, precision)) < 0)
        goto fail;
    }
  }
...
gauss_solve(Matrix       a,
  ...
  if (gauss_reduce(aug, precision, partial_pivot) < 0)
    goto fail;

  if (gauss_back_sub(aug, x_ptr, precision) < 0)
    goto fail;
