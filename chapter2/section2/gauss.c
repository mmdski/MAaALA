...
int
exchange_pivot_zero(Matrix m, size_t pivot_row, size_t pivot_col) {
  ...
  if (matrix_get(m, pivot_row, pivot_col, &pivot_value) < 0)
    return -1;
  ...
  if (matrix_row_exchange(m, pivot_row, i_nonzero) < 0)
    return -1;
  else
    return 0;
}
...
int
exchange_pivot_max(Matrix m, size_t pivot_row, size_t pivot_col) {
...
gauss_row_scale(Matrix aug) {
  ...
  for (size_t i = 1; i <= n_rows; i++) {
    ...
    if (matrix_row_mult(aug, i, 1. / max_row_value) < 0)
      return -1;
  }
...
gauss_reduce(Matrix a) {
  ...
  for (size_t pivot_col = 1, pivot_row = 1;
       pivot_col <= n_cols && pivot_row <= n_rows;
       pivot_col++) {

    if (exchange_pivot_zero(a, pivot_row, pivot_col) < 0)
      return -1;

    if (matrix_get(a, pivot_row, pivot_col, &pivot_value) < 0)
      return -1;
    ...
    for (size_t i = pivot_row + 1; i <= n_rows; i++) {
      ...
      if (matrix_get(a, i, pivot_col, &row_value) < 0)
        return -1;
      ...
      if (matrix_row_add_row(a, i, pivot_row, multiplier) < 0)
        return -1;
    }
...
gauss_jordan_reduce(Matrix a) {
  ...
  for (size_t pivot_col = 1, pivot_row = 1;
    ...
    pivot_value = values[MAT_INDEX(n_cols, pivot_row, pivot_col)];

    if (pivot_value == 0) {
      continue;
    }

    if (matrix_row_mult(a, pivot_row, 1. / pivot_value) < 0)
      return -1;

    for (size_t i = pivot_row + 1; i <= n_rows; i++) {

      row_value = values[MAT_INDEX(n_cols, i, pivot_col)];
